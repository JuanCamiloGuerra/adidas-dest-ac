...
.page{{display:none}}.page.active{{display:block}}[hidden]{{display:none!important}}.shell{{max-width:1480px;margin:auto;padding:24px}}.scenario-bar{{background:#fff;border:2px solid #000;padding:14px 16px;display:flex;align-items:center;justify-content:space-between;gap:20px;position:sticky;top:58px;z-index:40}}
...
<main class="shell">
<div class="scenario-bar" id="scenarioControls"><div class="scenario-copy"><b>Vista activa de cantidades y valor bruto</b><small id="scenarioDescription"></small></div><div class="switch" id="scenarioSwitch"><button data-scenario="original" class="active">Original</button><button data-scenario="minimum">Mínimo</button><button data-scenario="rf">Random Forest</button><button data-scenario="maximum">Máximo</button></div></div>
<section class="filters" id="dimensionFilters"><label>País<select id="country"></select></label><label>Canal<select id="channel"></select></label><label>Categoría<select id="category"></select></label><label>Segmento<select id="price_segment"></select></label><label>Estado<select id="order_status"></select></label><label>Periodo<select id="year_month"></select></label></section>
