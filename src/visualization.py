...
@media(max-width:600px){{.topbar{{padding:0 12px;overflow-x:auto}}.wordmark{{display:none}}.tabs{{min-width:max-content}}.hero{{padding:45px 18px}}.shell{{padding:12px}}.filters{{grid-template-columns:repeat(2,1fr)}}.switch{{grid-template-columns:repeat(2,1fr)}}.kpis,.comparison-strip{{grid-template-columns:1fr}}.section-head{{display:block}}.section-head p{{text-align:left;margin-top:10px}}.finding{{grid-template-columns:50px 1fr}}.finding-no{{font-size:32px}}}}
</style><script>{get_plotlyjs()}</script></head><body>
<header class="topbar"><div class="wordmark">SPORTRETAIL <small>LAM INTELLIGENCE STUDIO</small></div><nav class="tabs"><a class="summary-link" href="fase-1-api-tabla-maestra.html">Fase 1</a><a class="summary-link" href="presentacion-ejecutiva.html">Presentación ejecutiva</a><button class="tab active" data-page="overview">Desempeño</button><button class="tab" data-page="scenarios">Escenarios</button><button class="tab" data-page="decisions">Decisiones & ML</button><a class="summary-link" href="executive_summary.html">Resumen</a></nav></header>
